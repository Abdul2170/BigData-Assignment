...
public class BigDataTest extends Configured implements Tool {
    ...
    public static class TransposeMapper extends MapReduceBase implements Mapper<LongWritable, Text, LongWritable, Text> {
        ...
        public void map(LongWritable key, Text value, OutputCollector<LongWritable, Text> outputCollector, Reporter reporter) throws IOException {
            ...
            long rowNumber = key.get();

            for (String cell : value.toString().split(" ")) {
                outputCollector.collect(new LongWritable(column), new Text(rowNumber + "\t" + cell));
                ++column;
            }
        }
    }
    ...
    public static class TransposeReducer extends MapReduceBase implements Reducer<LongWritable, Text, Text, NullWritable> {
        ...
        public void reduce(LongWritable key, Iterator<Text> values, OutputCollector<Text, NullWritable> outputCollector, Reporter reporter) throws IOException {
            ...
            outputCollector.collect(new Text(rowString), NullWritable.get());
        }
    }
...
//        JobConf transposeMapperConf = new JobConf(false);
//        transposeMapperConf.setOutputKeyClass(LongWritable.class);
//        transposeMapperConf.setMapOutputValueClass(Text.class);
//        ChainMapper.addMapper(job, TransposeMapper.class, LongWritable.class, Text.class, LongWritable.class, Text.class, true, transposeMapperConf);
//
//        JobConf transposeReducerConf = new JobConf(false);
//        transposeReducerConf.setOutputKeyClass(LongWritable.class);
//        transposeReducerConf.setMapOutputValueClass(Text.class);
//        ChainReducer.setReducer(job, TransposeReducer.class, LongWritable.class, Text.class, Text.class, NullWritable.class, true, transposeReducerConf);
